<!-- Animated Header -->
![header](https://capsule-render.vercel.app/api?type=transparent&height=300&text=Aniket%20Dubey&textBg=false&animation=scaleIn&descAlignY=48&reversal=true&fontColor=FCF8F8)

<!-- Typing Animation -->
<p align="center">
  <img src="https://readme-typing-svg.demolab.com?font=Fira+Code&size=22&pause=1000&color=36BCF7&center=true&vCenter=true&width=600&lines=Backend+Focused+Developer;Systems+%26+Problem+Solving+Enthusiast;Go+%7C+Fiber+%7C+MongoDb+%7C+Docker+%7C+Linux" />
</p>

---

## 👋 About Me

I’m a **backend-focused developer** who believes in **strong fundamentals, clean architecture, and scalable systems**.  
I enjoy working close to the system — writing efficient code, understanding how things work under the hood, and building APIs that actually hold up under load.

- 💻 Comfortable with **low-level & high-level languages**
- ⚙️ Strong focus on **backend systems & APIs**
- 🐳 Prefer **Dockerized**, production-style setups
- 🐧 Linux user — CLI over GUI, always
- 📦 Databases, caching & performance optimization excite me

I believe good software is built the **traditional way**:  
> solid basics → clear logic → real-world constraints → then optimization.

---

## 🛠️ Tech Stack & Tools

### 💻 Programming Languages
![C](https://img.shields.io/badge/C-00599C?style=flat&logo=c&logoColor=white)
![C++](https://img.shields.io/badge/C++-004482?style=flat&logo=cplusplus&logoColor=white)
![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white)
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)

---

### 🌐 Frontend
![React](https://img.shields.io/badge/React.js-20232A?style=flat&logo=react&logoColor=61DAFB)

---

### 🌐 Backend
![Go](https://img.shields.io/badge/Go-00ADD8?style=flat&logo=go&logoColor=white)
![Fiber](https://img.shields.io/badge/Fiber-00ACD7?style=flat&logo=go&logoColor=white)

---

### 🗄️ Databases & Cache
![MongoDB](https://img.shields.io/badge/MongoDB-4EA94B?style=flat&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/Redis-D82C20?style=flat&logo=redis&logoColor=white)

---

### 🧰 Tools & DevOps
![Docker](https://img.shields.io/badge/Docker-2496ED?style=flat&logo=docker&logoColor=white)
![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white)
![Linux](https://img.shields.io/badge/Linux-FCC624?style=flat&logo=linux&logoColor=black)

---

### 🔍 API Testing
![Postman](https://img.shields.io/badge/Postman-FF6C37?style=flat&logo=postman&logoColor=white)
![Thunder Client](https://img.shields.io/badge/Thunder_Client-6C63FF?style=flat)

---

## 🚀 What I Like Building
- REST APIs with proper authentication (JWT, sessions)
- URL shorteners, management systems, backend services
- Redis-based caching for performance gains
- Dockerized applications ready for deployment
- Clean, readable code that another dev can actually understand

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats-fast.vercel.app/api/top-langs?username=AniketDubey199&layout=compact&langs_count=8" />
</p>

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=AniketDubey199&theme=tokyonight" />
</p>

---

## 📫 Connect With Me
- 💼 LinkedIn: https://linkedin.com/in/aniket-dubey-ad8889/
- 📧 Email: dubeyaniket7839@gmail.com

---

<!-- Footer -->
![footer](https://capsule-render.vercel.app/api?type=waving&color=gradient&height=120&section=footer)
